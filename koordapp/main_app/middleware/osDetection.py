...
class OsDetectionMiddleware:
    ...
    def __call__(self, request):
        ...
        try:
            ...
            pass
        if not 'Android' in user_agent and not 'iPhone' in user_agent and not 'iPad' in user_agent:    #IOS for debugging and testing    
            if not (path in allowed_urls_web or name_path in allowed_urls_web):
                return HttpResponseRedirect(reverse(main_url_web)) 
        elif 'Android' in user_agent or 'iPhone' in user_agent or 'iPad' in user_agent:        #IOS for debugging and testing
